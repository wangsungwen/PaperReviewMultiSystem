...
if entry_mode == "⚙️ 管理員 (參數設定)":
    ...
    with col_b:
        ...
        active_config["local"]["n_ctx"] = st.number_input("上下文窗口大小 (n_ctx)", value=active_config["local"].get("n_ctx", 37888), step=1024)
        active_config["local"]["max_tokens"] = st.number_input("最大輸出 Token", value=active_config["local"].get("max_tokens", 12288), step=256)
...
# ----------------- 模組 2：推論主畫面 -----------------
    ...
    with col_t1:
        ...
        with c2: 
            if st.button("🔍 檢測推論硬體狀態", use_container_width=True):
                with st.spinner("正在檢測硬體資源..."):
                    ...
                    if llm_mode in ["cloud", "gemini_native"]:
                        llm_hw_status = "☁️ Cloud API (線上引擎)"
                    elif llm_mode == "ollama":
                        llm_hw_status = "🦙 Ollama Server (高併發處理)"
                    elif llm_mode == "local":
                        n_gpu = active_config.get("local", {}).get("n_gpu_layers", 0)
                        llm_hw_status = f"💻 GPU (Offloaded {n_gpu} layers)" if n_gpu != 0 else "💻 CPU"
                    else: 
    ...
    col_title, col_field = st.columns([0.6, 0.4])
    with col_title:
    ...
    raw_paper_content = st.text_area("原始論文內容 (上傳後將顯示於此，或可直接貼上)", height=200, key="paper_content_text_area")
    ...
    with col_s1:
        ...
        exclusion_keywords = st.text_area(
            ...
            value=st.session_state.exclusion_keywords, height=120
        )
        ...
        with c_m2:
            st.write(" "); st.write(" ")
            if st.button("加入排除") and manual_exclude_input and manual_exclude_input not in st.session_state.manual_exclusions:
    ...
    if filtered_text:
        ...
        if exclusion_keywords.strip():
            for kw in [k.strip() for k in exclusion_keywords.split('\n') if k.strip()]:
                idx = filtered_text.lower().find(kw.lower().strip())
                if idx != -1:
    ...
    with col_d1:
        if st.button("🔎 執行 AI 寫作分析", type="primary"):
    ...
    if st.session_state.ai_report:
        ...
        html_box = "<div style='line-height:1.8; border:1px solid #ddd; padding:20px; border-radius:10px; background-color:#fafafa; color:#333; font-size:16px;'>"
        for s in report['segments']:
            style = f"background-color:{s['color']}; border-radius:3px; color:#000;" if s['type'] == 'AI' else "color:#333;"
            html_box += f"<span style='{style}'>{s['text']}</span>"
        html_box += "</div>"
    ...
    with st.expander("➕ 新增/管理代理人委員", expanded=False):
        ...
        for i, r in enumerate(st.session_state.reviewers):
            with cols[i]:
                st.info(f"**{r.name}**\n\n專業：{r.expertise}\n\n風格：{r.style}")
        st.divider()
    ...
    async def run_review():
        ...
        orchestrator = PaperReviewOrchestrator(
            Paper(paper_title, paper_field, final_paper_content_for_llm), 
            st.session_state.reviewers, 
            llm_service
        )
